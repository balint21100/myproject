...
{
    public partial class Form3nagyssz : Form
    {
        #region valtozok
        ...
        public string masodlagoslista = "";
        public List<string> masodlagoslista2 = new List<string>();
        ...
        private void menu_Click(object sender, EventArgs e)
        {
            ...
            {
                uditogombok[i].BackColor = Color.Chocolate;
                uditogombok[i].Visible = true;
            }
            kozepes_menu.BackColor = Color.Orange;
            nagy_menu.BackColor = Color.Chocolate;
            label4.Visible = true;
            meret = "Közepes";
            uditomeret = "0,4 L";
        ...
        {
            kozepes_menu.BackColor = Color.Chocolate;
        ...
        private void udito_kivalasztas(object sender, EventArgs e)
        {
            ...
            kivalasztott_udito.BackColor = Color.Chocolate;
        ...
        private void button16_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < masodlagoslista2.Count; i++)
            {
                f1.listBox2.Items.Add(masodlagoslista2[i]);
            }
            f1.vegosszeg_form1 +=vegar;
            string vegosszesen_szovegkent = "";
            vegosszesen_szovegkent = vegar.ToString();
            f1.belepettfelhasznalo = felhasznalon.Tag.ToString();
            f1.btn3.Tag = felhasznalon.Tag;
            f1.felhasznalo.Text = felhasznalon.Tag.ToString();
        ...
        private void Beolvasas_nagyszendvicsek_tabla(string valtozo)
        {
            ...
            sql = $"SELECT * FROM nagyszendvicsek WHERE nagysz_nev = '{valtozo}'";
            ...
            mennyiseglbl.Text = "1";
            kiirtnev.Text = $"{nev} {mennyiseglbl.Text} db {csar * mennyiseg} Ft";
            masodlagoslista = $"{nev} {mennyiseglbl.Text} db";
            ...
            sql = $"SELECT nev, leiras FROM minden_leirasa WHERE nev = '{valtozo}'";
            command = new SqlCommand(sql, connection);
            datareader = command.ExecuteReader();
            datareader.Read();
            //label3.Text = "";
            label3.Text = $"{datareader.GetValue(0)} - {datareader.GetValue(1)}";
        ...
        private void uditocreator()
        {
            ...
            {
                /*Button tmd = new Button();
                tmd.Size = new Size(146, 66);
                tmd.BackColor = Color.DarkOrange;
                tmd.Text = item;
                tmd.Font = new Font("Calibri", 20);
                int x = (szamlepegeto % 3) + 6;
                int y = (szamlepegeto / 3) + 1 * n;
                tmd.Location = new Point(x, y);
            ...
            n = 70;
            for (int i = 0; i < uditolist.Count; i++)
            {
                Button tmd = new Button();
                tmd.Size = new Size(160, 66);
                tmd.BackColor = Color.DarkOrange;
                tmd.Text = uditolist[i];
                tmd.Font = new Font("Calibri", 16);
                int x = (i % 3) * 170;
                int y = (i / 3)* n;
                tmd.Location = new Point(x, y);
                tmd.Click += new EventHandler(uditolenyomas);
                tmd.Name = uditolist[i];
                tmd.Visible = false;
                uditogombok.Add(tmd);
                panel2.Controls.Add(tmd);
                //nagyszendvicsgombok.Add(tmd);
